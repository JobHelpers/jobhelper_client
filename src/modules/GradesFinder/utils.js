...
export const joinGroupedSpecialitiesSubjects = (specialitiesSubjects) => {
  ...
  specialitiesSubjects.forEach(item => {
    ...
    switch (item.subject_status) {
      case "optional":
        modifiedSpecialitiesSubjects[item.speciality_code][2] = item.subject_ids.split(',')
        break;
      case "mandatory":
        modifiedSpecialitiesSubjects[item.speciality_code][1] = item.subject_ids.split(',')
        break;
      case "main":
      default:
        modifiedSpecialitiesSubjects[item.speciality_code][0] = item.subject_ids.split(',')
        break;
    }
  });
...
export const specialityHasSubjects = (specialitiesSubjects, specialityCode, mainSubject, subjects) => {
  ...
  // check main
  if (!mainSubjects.includes(mainSubject) && selectedSpecialitiesSubjects[0][0] !== 1) return false;
